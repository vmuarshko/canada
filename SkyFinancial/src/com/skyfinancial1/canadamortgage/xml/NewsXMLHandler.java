	...
	public void characters(char ch[], int start, int length) throws SAXException 
	{
		...
		{			
			if (!enclosingTags.isEmpty() && blogEntries.size() > 0)
			{
				...
				{
					blogEntries.get(blogEntries.size() - 1).title = new String(ch, start, length);					
				}
				else if (NewsXMLTags.itemPubDate.equals(enclosingTags.peek()))
				{
					blogEntries.get(blogEntries.size() - 1).pubDate = new String(ch, start, length);					
				} 
				else if (NewsXMLTags.itemDescription.equals(enclosingTags.peek()))
				{
					...
					{
						if (blogEntries.get(blogEntries.size() - 1).desctiption == null)
							blogEntries.get(blogEntries.size() - 1).desctiption = desctiption;
						else
							blogEntries.get(blogEntries.size() - 1).desctiption += desctiption;
					}
					
					blogEntries.get(blogEntries.size() - 1).desctiption = new String(ch, start, length);					
				} 
				else if (NewsXMLTags.itemContent.equals(enclosingTags.peek()))
				{
					...
					{
						if (blogEntries.get(blogEntries.size() - 1).content == null)
